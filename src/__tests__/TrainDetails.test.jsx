...
describe('TrainDetails Component', () => {
  ...
  test('renders train details and handles class selection', async () => {
    ...
    mockAxios
      .onGet('/api/search/train-details', { params: { scheduleId: 'schedule123', fromStopId: 'from123', toStopId: 'to123' } })
      .reply(200, {
        fromStation: 'New York',
        toStation: 'Washington',
        coachTypes: [
          { _id: 'class1', name: 'First Class', priceFactor: 1.5, facilities: ['AC', 'Wifi'], available: true },
          { _id: 'class2', name: 'Second Class', priceFactor: 1, facilities: ['Food', 'Wifi'], available: false },
        ],
      });
  ...
  test('displays loading state while fetching train details', () => {
    mockAxios.onGet('/api/search/train-details').reply(200, {}); // Simulate slow API response
  ...
  test('handles API error gracefully', async () => {
    // Mock an API error
    mockAxios.onGet('/api/search/train-details').reply(500);
