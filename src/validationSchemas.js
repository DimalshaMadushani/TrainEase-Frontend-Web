...
export const checkoutSchema = yup.object().shape({
  ...
  expiryDate: yup.string()
    ...
    .test('is-expiry-valid', 'Expiry date is invalid or has passed', value => {
      ...
      const currentMonth = currentDate.getMonth() + 1; // January is 0, need +1 to align with human-readable months (1-12)
      const [expMonth, expYear] = value.split('/').map(Number);
      if (expYear < currentYear) return false; // Year has definitely passed
