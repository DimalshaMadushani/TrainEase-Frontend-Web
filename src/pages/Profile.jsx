...
const Profile = () => {
    ...
    useEffect(() => {
        // Fetch user data
        axios.get(`http://localhost:5000/profile/${formData.username}`)
                    ...
                    {sortedBookingHistory.map((booking, index) => (
                        <Paper key={index} elevation={3} sx={{ display: 'flex', justifyContent: 'space-between', padding: '16px', mb: 2 }}>
                            <Box>
                                ...
                                <Typography variant="body2">Passengers: {booking.passengers}</Typography>
                                <Typography variant="body2">Total Amount: {booking.amount}</Typography>
                            </Box>
